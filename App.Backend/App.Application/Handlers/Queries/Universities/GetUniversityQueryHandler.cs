...
public class GetFacultyQueryHandler (IUnitOfWork unitOfWork) : IRequestHandler<GetUniversityQuery, Result<UniversityDetailResponse>>
{
    ...
    public async Task<Result<UniversityDetailResponse>> Handle(GetUniversityQuery request, CancellationToken cancellationToken)
    {
        ...
        if (university == null)
            return Result.Failure<UniversityDetailResponse>(UniversityErrors.NotFound);
